<script context="module">
    ...
    export const OIDC_CONTEXT_CLIENT_PROMISE = writable({});
    export const OIDC_CONTEXT_REDIRECT_URI = writable({});
    export const OIDC_CONTEXT_POST_LOGOUT_REDIRECT_URI = writable({});
    ...
    export async function refreshToken() {
        try {
            ...
            const oidc = await get(OIDC_CONTEXT_CLIENT_PROMISE);
    ...
    export async function login(preserveRoute = true, callback_url = null) {
        const oidc = await get(OIDC_CONTEXT_CLIENT_PROMISE);
        const redirect_uri = callback_url || get(OIDC_CONTEXT_REDIRECT_URI) || window.location.href;
    ...
    export async function logout(logout_url = null) {
        const oidc = await get(OIDC_CONTEXT_CLIENT_PROMISE);
        const returnTo = logout_url || get(OIDC_CONTEXT_POST_LOGOUT_REDIRECT_URI) || window.location.href;
